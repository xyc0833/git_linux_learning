## 📚 Git学習リポジトリ / Git Learning Repository / Git学习仓库


プログラミング初心者のじょ ようしん（徐耀晨）です。  
Gitの基本操作（commit/push/pull等）を学習中です。

### 🔹 現在の学習内容
- `git commit`：ローカルリポジトリに変更を保存
- `git push`：リモートリポジトリ（GitHub）にアップロード
- `git pull`：リモートの最新変更をローカルに取得

### 📌 注意事项
- 初めての操作はテストブランチで実施しています
- コミットメッセージは日本語で記述中（次週から英語移行予定）

I'm XU Yaochen (xyc), a programming beginner.  
Currently learning Git fundamental operations including commit/push/pull.

### 🔹 Learning Progress
- `git commit`: Save changes to local repository
- `git push`: Upload to remote repository (GitHub)
- `git pull`: Fetch latest changes from remote

### 📌 Notes
- Practicing on test branches for first-time operations
- Commit messages currently in Japanese (planning to switch to English next week)

我是编程新手徐耀晨（xyc）。  
正在学习Git基础操作，包括commit/push/pull等命令。

### 🔹 当前学习进度
- `git commit`：将更改保存到本地仓库
- `git push`：上传到远程仓库（GitHub）
- `git pull`：获取远程最新更改
...
- [ ] **日文版简历修改 寻求帮助** - 
  - 描述：1 字体方面的建议 一般日本人word里面常用什么类型的字体
    2 日本人的新卒 比方说 it相关专业的同学 比较优秀的简历是什么样的 有无范本
    3 工作经历和 相对应的开发技能 要怎么写 才算是比较优秀的 （国内可能会偏复杂一点 日本那边是不是会要求更清晰简单一点 而不是写复杂？）
    3 自己的简历不足和踩雷的地方 在哪里
    4 职务经历里面可以写项目经历吗？ 项目经历的话 怎么标注为项目经历 不是工作经历（主要是这个地方要填会社名 我这个也填不了会社名）
    5 想问一下 履历书的话 如果只有实习的经历 一般情况下 职历这里写不写的？ 職　歴 这里 怎样标注是实习？
    6 25年6月份毕业  25年 8月到 26年的1月份（后面离职了） 实际上是在干销售的 （但是这段经历不想写到履历书里面 因为和IT开发的岗位完全不相关 ）能不能在这段时间把自己学习的项目 填上去 ？
    7 学历和 职历 第一行 怎么写？
    8 有些单词的翻译 比方it相关术语的翻译 就比较奇怪 我也拿不准 （虽然用的是ai）
    9 在国内交过社保 然后离职了 （在日本无工作经验） 算第二新卒还是 既卒 还是算中途？
  - [ ] 履历书
  - [ ] 职务经验书
...
- [x] **周日的笔试题 — 三道编程题 复习**
  - 描述：wxchat 链接
  - [x] leetcode LRU缓存 c++的写法 + java的写法
  - [x] 字符串那道题 学习 java的写法
  - [x] 最大字数组和  学习java的写法
- [x] **廖雪峰 JavaSE 加密与安全**
  - 描述：还是看一下吧 万一工作会用呢？
...
- [ ] **leetcode hot 100** 
    - 描述：前面有一部分题目刷过的 忘记掉了 
    - 想用java的方式再复习一下 顺便熟练一下 java相关的基础用法
- [ ] **github action视频**
  - 描述：B站 傻子 git的最后一个教程 
- [ ] **typescript 学习视频**
